...
def test_tree_size():
    assert tree.Tree('(a:4)c:2;').size == (6, 1)
    assert tree.Tree('(a:4,b:5)c:2;').size == (7, 2)


def test_copy():
    for tree_text in good_trees:
        t1 = tree.Tree(tree_text)
        t2 = t1.copy()
...
def test_getitem():
    t = tree.Tree('((d:8,e:7)b:6,(f:5,g:4)c:3)a:2;')
    assert t['a'] == t
