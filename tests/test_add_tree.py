...
def test_add_trees_to_db():
    ...
    with NamedTemporaryFile() as fp:
        exec(f'sqlite3 {fp.name} < {PATH}/scripts/create_tables.sql')
        exec(f'sqlite3 {fp.name} < {PATH}/scripts/sample_data.sql')

        exec(f'{PATH}/scripts/add_tree.py --db {fp.name} --name with_test '
                f'{PATH}/examples/HmuY.aln2.tree')

        add_all(db=fp.name)

        with pytest.raises(AssertionError):
            exec(f'{PATH}/scripts/add_tree.py --db {fp.name} nonexistent_file')
